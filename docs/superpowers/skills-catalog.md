...
> Оновлено: 2026-07-31. Джерело: `.agents/skills/` (проєктні, симлінки в
> `.claude/skills/`), `docs/superpowers/skills/` (авторовані в репо,
> відстежувані) та плагін superpowers v6.2.0. Колонка «Роль у
> пайплайні» — за спекою
...
## Авторовані в репо (`docs/superpowers/skills/`)

Єдині скіли, чиє джерело відстежується git-ом; решта приходить менеджером або
плагіном. Правки вносяться в `docs/superpowers/skills/`, робоча копія
встановлюється командою з [`skills/README.md`](skills/README.md) — агента в
`.claude/` і `.agents/` не пускає `agent-guard`.

| Скіл | Що робить | Режим | Виклик | Роль у пайплайні |
|------|-----------|-------|--------|------------------|
| [`animating-svg-for-web`](skills/animating-svg-for-web/SKILL.md) | Анімовані SVG-діаграми та векторні ілюстрації для вебсторінок: гейт частоти показу, семантичний контракт SVG (шари, `data-*`, `pathLength`, `useId`-префікси), драбина технологій CSS → WAAPI → motion → GSAP, пʼятиактна хореографія, верифікація на заморожених кадрах. Окремий довідник ізометричних лінійних систем (осі, пунктир+маска, орбіти, exploded view, штрихування у світлій зоні) | write | авто (модельний) | animation-engineer — для діаграм та ілюстрацій; frontend-implementer (розмітка SVG-компонента) |
...
- Виняток — `docs/superpowers/skills/`: власні скіли відстежуються там, а
  `.claude/skills/` тримає лише робочу копію. Розбіжність між джерелом і копією
  git не покаже — після правки джерела копію переставляють вручну.

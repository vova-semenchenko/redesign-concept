...
> пайплайні» описує ролі агентного UI-пайплайну — його спека й реалізація
> більше не зберігаються в репозиторії.
...
- `animating-svg-for-web` (анімовані SVG-діаграми та векторні ілюстрації) живе
  текою прямо в `.claude/skills/` і **джерела в репо не має** — свідоме рішення
  від 2026-07-31. Отже: у клоні репо його не буде, `git status` про його зміни
  не скаже, і `rm -rf .claude` знищить єдиний екземпляр. Перед такими
  операціями копіювати теку вручну.
